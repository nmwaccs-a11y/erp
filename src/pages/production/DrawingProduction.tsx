...
export default function DrawingProduction() {
    ...
    useEffect(() => {
        const lastId = localStorage.getItem("lastDrawingProductionId") || "0";
        setNextProductionId(parseInt(lastId) + 1);
    }, []);

    const handleAddEntry = () => {
        if (!machineNo || !inputWire || !outputWire || !weight) {
        ...
        // Keep Machine and Wires for faster entry
    ...
    const handleSaveBatch = () => {
        ...
        const batchId = `DP-${nextProductionId.toString().padStart(5, "0")}`;

        // Save last ID
        localStorage.setItem("lastDrawingProductionId", nextProductionId.toString());
            ...
            <div className="space-y-6 max-w-4xl mx-auto">
                <div className="flex items-center gap-4">
                    ...
                    <div>
                        ...
                        <p className="text-slate-500">Record wire drawing process output. Batch ID increments once per save.</p>
                    </div>
                </div>
                        ...
                        <CardContent className="p-6 space-y-4">
                            ...
                            <div className="grid gap-2">
                                ...
                                <Select value={machineNo} onValueChange={setMachineNo}>
                                    ...
                                    <SelectContent>
                                        <SelectItem value="D-01">Drawing M-01 (Fine)</SelectItem>
                                        <SelectItem value="D-02">Drawing M-02 (Medium)</SelectItem>
                                        <SelectItem value="D-03">Drawing M-03 (Rod Breakdown)</SelectItem>
                                    </SelectContent>
                                </Select>
                            </div>
                            ...
                            <div className="grid grid-cols-2 gap-4">
                                <div className="grid gap-2">
                                    <Label htmlFor="inputWire">In (mm)</Label>
                                    <Input
                                        id="inputWire"
                                        placeholder="e.g. 8.0"
                                        value={inputWire}
                                        onChange={(e) => setInputWire(e.target.value)}
                                    />
                                </div>
                                <div className="grid gap-2">
                                    <Label htmlFor="outputWire">Out (mm)</Label>
                                    <Input
                                        id="outputWire"
                                        placeholder="e.g. 2.5"
                                        value={outputWire}
                                        onChange={(e) => setOutputWire(e.target.value)}
                                    />
                                </div>
                            </div>
                    ...
                    <Card className="lg:col-span-2 border-slate-200 shadow-sm flex flex-col">
                        <CardHeader className="bg-slate-50 border-b border-slate-100 pb-4 flex flex-row items-center justify-between">
                            <div>
                                ...
                                <CardDescription>Batch ID: DP-{nextProductionId.toString().padStart(5, "0")}</CardDescription>
                            </div>
                                ...
                                <Table>
                                    <TableHeader>
                                        <TableRow>
                                            ...
                                            <TableHead>Specs (In/Out)</TableHead>
                                    ...
                                    <TableBody>
                                        {entries.map((entry) => (
                                            <TableRow key={entry.id}>
                                                ...
                                                <TableCell>{entry.inputWire} / {entry.outputWire} mm</TableCell>
                        ...
                        <div className="p-4 border-t border-slate-100 bg-slate-50/50">
                            ...
                            >
                                ...
                                Save Batch (ID: DP-{nextProductionId.toString().padStart(5, "0")})
                            </Button>
                        </div>
                    </Card>
                </div>
